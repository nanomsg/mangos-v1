...
// GLOBALS
var slept bool // used by portHook to sleep only during first time a port is added

// portHook is a simple port hook that just prints when a port/endpoint is added/removed.
// Use the environment variable $PORTSLEEP (time.Duration value) to sleep for a specified time
// when the port is added. This is helpful when simulating dropping connections.
...
// Port2Str pretty-prints a mangos port address
